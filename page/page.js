...
var html_obj = document.getElementsByClassName("html");
...
function keyboardInput(event) {
	...
	if (["C", "c", "Delete", "delete", "Backspace", "backspace"].includes(input)) {
		addChar("C");
	}
	else if (all_inputs["char"].includes(input)) {
		addChar(String(event.key));
	}
	...
	else if(["=", "Enter", "enter"].includes(input)) {
		addChar("=");
	}
}

html_obj[0].addEventListener("keydown", 
	(event) => {
		keyboardInput(event);
	}
);